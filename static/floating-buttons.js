/* ConvertAR - Floating Buttons v1 (WA + Promos popup) */
(function(){
  ...
  var DEFAULT_PROMOS=[
    {icon:'\u1F4B3',titulo:'6 cuotas sin inter\u00E9s',detalle:'Con todas las tarjetas'},
    {icon:'\u1F3E6',titulo:'10% OFF con transferencia',detalle:'Mejor precio al momento de pagar'},
    {icon:'\u1F69A',titulo:'Env\u00EDo gratis',detalle:'En compras desde $69.999'}
  ];
  var _promos=null;
  ...
  var CSS=[
    ...
    '#ph-fb-modal{background:#fff;width:100%;max-width:400px;border-radius:20px 20px 0 0;padding:0;overflow:hidden;animation:ph-fb-up .3s cubic-bezier(.2,.8,.3,1)}',
    '@media(min-width:480px){#ph-fb-modal{border-radius:20px;max-height:90vh}}',
    ...
    '.ph-fb-mclose{background:rgba(255,255,255,.15);border:none;color:#fff;width:28px;height:28px;border-radius:50%;cursor:pointer;font-size:16px;display:flex;align-items:center;justify-content:center;line-height:1}',
    '.ph-fb-mclose:hover{background:rgba(255,255,255,.25)}',
    '.ph-fb-mbody{padding:16px 20px 24px;display:flex;flex-direction:column;gap:12px}',
  ...
  promoBtn.innerHTML='\u1F525 Ver promos hoy';
  ...
  function init(){document.body.appendChild(wrap);}
  ...
  function renderModal(promos){
    ...
    htit.textContent='Promociones de hoy \u1F389';
