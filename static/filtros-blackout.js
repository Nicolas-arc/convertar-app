/* ============================================================
   ...
   Lee la configuración de /config/:shop — editable desde el panel.
...
(function () {
  ...
  var CAT_PATH = '/black-out';

  /* Config compartida con el resto de los scripts CVA */
  ...
  /* Filtros por defecto (fallback si no hay config) */
  var DEFAULT_FILTROS = [
  ...
  function aplicarFiltro(key, url) {
    ...
    getCards().forEach(function(card) {
      var m = matchFiltro(getNombre(card), key);
      card.style.display = m ? '' : 'none';
      if (m) visibles++;
    });
    document.querySelectorAll('.ph-co-pill').forEach(function(p) {
      p.classList.toggle('active', p.dataset.key === key);
    });
    var res = document.getElementById('ph-cortinas-result');
    if (res) res.textContent = visibles + ' producto' + (visibles !== 1 ? 's' : '');
  }

  function insertarHero(filtros, heroTitulo) {
    if (document.getElementById('ph-cortinas-hero')) return;

    var st = document.createElement('style'); st.textContent = css; document.head.appendChild(st);

    var titulo = heroTitulo || '¿Cuál es la cortina perfecta para tu ventana?';
    ...
    hero.innerHTML = '<div id="ph-cortinas-hero-inner">'
      ...
      + '<h1>' + titulo + '</h1>'
      + '<div class="ph-cortinas-formula">'
      +   '<div class="ph-cortinas-fbox"><span class="flabel">Alto</span><span class="fval">Tu ventana</span></div>'
      +   '<span class="ph-cortinas-fsep">×</span>'
      +   '<div class="ph-cortinas-fbox"><span class="flabel">Ancho</span><span class="fval">Cantidad de paños</span></div>'
      +   '<span class="ph-cortinas-fsep">=</span>'
      +   '<div class="ph-cortinas-fbox"><span class="flabel">Tu cortina</span><span class="fval">Perfecta</span></div>'
      + '</div>'
      + '<p class="ph-cortinas-note">El <strong>alto</strong> define el tamaño · El <strong>ancho</strong> define cuántos paños. Cada paño mide 130cm de ancho.</p>'
      + '</div>';

    var pillsHtml = filtros.map(function(f) {
      return '<button class="ph-co-pill' + (f.key === 'todos' ? ' active' : '') + (f.extraClass ? ' ' + f.extraClass : '') + '"'
        + ' data-key="' + f.key + '" data-url="' + (f.url || '') + '">'
        + '<span class="ph-co-label">' + (f.label || f.key) + '</span>'
        + '<span class="ph-co-sub">' + (f.sub || '') + '</span>'
        + '</button>';
    }).join('');

    var filtrosEl = document.createElement('div'); filtrosEl.id = 'ph-cortinas-filtros';
    filtrosEl.innerHTML = '<h3>¿Qué alto tiene tu ventana?</h3><p>Seleccioná y te mostramos exactamente las que te quedan</p>'
      + '<div id="ph-cortinas-pills">' + pillsHtml + '</div>'
      + '<div id="ph-cortinas-result"></div>';
    ...
    document.querySelectorAll('.ph-co-pill').forEach(function(btn) {
      btn.addEventListener('click', function() { aplicarFiltro(this.dataset.key, this.dataset.url || null); });
    });
    aplicarFiltro('todos');
    new MutationObserver(function() {
      if (activoKey !== 'todos') aplicarFiltro(activoKey);
  ...
  function init() {
    window.__CVA_CFG_P.then(function(cfg) {
      ...
      if (f.filtros === false) return;

      /* Leer config de categoría desde el panel */
      var cats    = (cfg && cfg.categorias) || {};
      var catCfg  = cats[CAT_PATH] || {};
      var filtros = (catCfg.filtros && catCfg.filtros.length) ? catCfg.filtros : DEFAULT_FILTROS;
      var titulo  = catCfg.hero_titulo || null;

      /* Esperar a que exista el grid de productos */
      var elapsed = 0;
      var iv = setInterval(function() {
        elapsed += 100;
        var ref = document.querySelector('.js-product-table,.products-grid,.js-products-container,#products');
        if (ref) { clearInterval(iv); insertarHero(filtros, titulo); }
